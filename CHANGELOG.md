...
- Phase 12: documentation, public API baseline and packaging – a `README.md` written for someone who has not read the
  ...
  deterministic CI build; and `CONFORMANCE.md`, the traceability from the normative rules of S2 Connect and of
  PLAN.md §3.3 to the tests that exercise them, generated by `ConformanceDocumentTests` from the `[S2C]`
  properties the tests carry (219 rules across 18 areas, 589 references) and compared on every run like the API
  baseline, so a rule that loses its last test shows up as a diff. CI and nightly now follow the Hermod and Styx workflows (`windows-latest` and Debian 13 in a
