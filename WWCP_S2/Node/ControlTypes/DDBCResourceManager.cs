...
namespace cloud.charging.open.protocols.S2.Node
{
    ...
    /// <summary>
    /// A delegate called on the RM when the CEM sent a DDBC instruction.
    /// </summary>
    ...
    public delegate Task<ReceptionStatusValue?> OnDDBCInstructionDelegate(S2Session          Session,
                                                                          DDBC_Instruction   Instruction,
                                                                          CancellationToken  CancellationToken);


    /// <summary>
    /// The Resource Manager side of Demand Driven Based Control (S2 JSON DDBC): when the CEM selects
    /// DDBC, this handler sends the actuators that can serve the demand
    /// (<see cref="DDBC_SystemDescription"/>) and then forwards the CEM's
    /// <see cref="DDBC_Instruction"/>s to <see cref="OnInstruction"/>, answering each with an
    /// <see cref="InstructionStatusUpdate"/> of status NEW unless the callback returns a status.
    /// The application pushes <see cref="DDBC_ActuatorStatus"/>,
    /// <see cref="DDBC_PresentDemandStatus"/>, <see cref="DDBC_AverageDemandRateForecast"/>,
    /// <see cref="DDBC_TimerStatus"/> and power measurements through the session itself.
    /// </summary>
    public sealed class DDBCResourceManager : IS2ControlTypeHandler
    {

        #region Data

        private readonly Func<DDBC_SystemDescription>  systemDescriptionFactory;
        ...
        /// <summary>
        /// An event fired when the CEM sent a DDBC instruction; the handler answers with the
        /// returned reception status (null = OK).
        /// </summary>
        public event OnDDBCInstructionDelegate?  OnInstruction;
        ...
        /// <summary>
        /// Create a new DDBC resource manager handler with a fixed system description.
        /// </summary>
        /// <param name="SystemDescription">The DDBC system description sent when DDBC is activated.</param>
        /// <param name="TimeProvider">An optional time provider.</param>
        public DDBCResourceManager(DDBC_SystemDescription  SystemDescription,
        ...
        /// <summary>
        /// Create a new DDBC resource manager handler with a system description built on activation
        /// (e.g. with the current ValidFrom).
        /// </summary>
        /// <param name="SystemDescriptionFactory">A factory of the DDBC system description.</param>
        /// <param name="TimeProvider">An optional time provider.</param>
        public DDBCResourceManager(Func<DDBC_SystemDescription>  SystemDescriptionFactory,
                                   TimeProvider?                 TimeProvider   = null)
        {
        ...
        public IDisposable RegisterHandlers(S2Session Session)
            ...
            => Session.On<DDBC_Instruction>(async (session, instruction, ct) => {
