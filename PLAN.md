...
`FRBCResourceManager`/`FRBCEnergyManager` `IS2ControlTypeHandler`s (the PEBC, PPBC, OMBC and DDBC handlers followed
after phase 12, so the PV sample now runs PEBC); `JSONFileS2Store` reuses `InMemoryS2Store` via an internal snapshot/load seam and persists
